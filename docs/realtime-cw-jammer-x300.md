...
only with values from the active setup. `--confirm-controlled-rf`, explicit
`--gain`, and `--manifest` remain mandatory for live transmission.

In continuous mode, `jammertx` streams until `SIGINT`/`SIGTERM`, sends UHD
end-of-burst metadata, and writes the final manifest. The global `--ramp` is the
startup ramp; an operator stop has no scheduled sample-level fall ramp. Finite
mode remains available when exact onset, offset, and recovery sample boundaries
must be generated by the transmitter itself.
...
| `--manifest FILE` | required live | JSON evidence record, written before and after TX |
...
`jammertx` writes a JSON record before opening the hardware and overwrites it
with final results at shutdown. If startup fails or the process terminates
abnormally, an `incomplete` preflight record should remain.
...
### `live TX requires --manifest FILE`

Choose a writable evidence path. `jammertx` checks it before opening UHD.
