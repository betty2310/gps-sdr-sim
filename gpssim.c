...
void initAttackNoiseState(unsigned int noise_state[MAX_SAT]) {
  ...
  for (sv = 0; sv < MAX_SAT; sv++)
    noise_state[sv] = 0x9e3779b9U ^ ((unsigned int)(sv + 1) * 0x45d9f3bU);
...
const char *attackMethodName(attack_method_t method) {
  switch (method) {
  case ATTACK_METHOD_JAM_DROP:
    return ("jam_drop");
  case ATTACK_METHOD_JAM_NOISE:
    return ("jam_noise");
  case ATTACK_METHOD_SPOOF_DELAY:
    return ("spoof_delay");
  case ATTACK_METHOD_SPOOF_NAV:
    return ("spoof_nav");
  case ATTACK_METHOD_NONE:
  default:
    return ("normal");
  }
}
...
int parseAttackMethod(const char *name, attack_method_t *method) {
  if (strcmp(name, "normal") == 0)
    *method = ATTACK_METHOD_NONE;
  else if (strcmp(name, "jam_drop") == 0)
    *method = ATTACK_METHOD_JAM_DROP;
  else if (strcmp(name, "jam_noise") == 0)
    *method = ATTACK_METHOD_JAM_NOISE;
  else if (strcmp(name, "spoof_delay") == 0)
    *method = ATTACK_METHOD_SPOOF_DELAY;
  else if (strcmp(name, "spoof_nav") == 0)
    *method = ATTACK_METHOD_SPOOF_NAV;
  else
    return (FALSE);
...
int parseAttackConfig(attack_config_t *cfg, const char *spec) {
  ...
  while (token != NULL) {
    ...
    *colon = '\0';
    ...
    if (parseAttackMethod(colon + 1, &method) == FALSE)
      return (FALSE);
...
int hasUnimplementedAttack(const attack_config_t *cfg) {
  ...
  for (sv = 0; sv < MAX_SAT; sv++) {
    if (cfg->method[sv] != ATTACK_METHOD_NONE &&
        cfg->method[sv] != ATTACK_METHOD_JAM_DROP &&
        cfg->method[sv] != ATTACK_METHOD_JAM_NOISE)
      return (TRUE);
  }
...
attack_method_t getAttackMethod(const attack_config_t *cfg, int prn) {
  if (prn < 1 || prn > MAX_SAT)
    return (ATTACK_METHOD_NONE);

  return (cfg->method[prn - 1]);
}
...
void synthEphemeris(ephem_t *eph, const double *rx_xyz, double az, double el,
  ...
  eph->m0 =
      wrapToPi(uk); // mean anomaly = argument of latitude for circular orbit
  eph->aop = 0.0;
  eph->omg0 = wrapToPi(ok + OMEGA_EARTH *
                                toe.sec); // invert Earth rotation from satpos()
...
int readUserMotionLLH(double xyz[USER_MOTION_SIZE][3], const char *filename) {
  ...
  for (numd = 0; numd < USER_MOTION_SIZE; numd++) {
    ...
    if (EOF == sscanf(str, "%lf,%lf,%lf,%lf", &t, &llh[0], &llh[1],
                      &llh[2])) // Read CSV line
      break;

    if (llh[0] > 90.0 || llh[0] < -90.0 || llh[1] > 180.0 || llh[1] < -180.0) {
      fprintf(stderr, "ERROR: Invalid file format (time[s], latitude[deg], "
...
void usage(void) {
  fprintf(
      ...
      "(required)\n"
      "  -u <user_motion> User motion file in ECEF x, y, z format (dynamic "
      "mode)\n"
      "  -x <user_motion> User motion file in lat, lon, height format (dynamic "
      "mode)\n"
      "  -g <nmea_gga>    NMEA GGA stream (dynamic mode)\n"
      "  -c <location>    ECEF X,Y,Z in meters (static mode) e.g. "
      "3967283.154,1022538.181,4872414.484\n"
      "  -l <location>    Lat, lon, height (static mode) e.g. "
      "35.681298,139.766247,10.0\n"
      "  -L <wnslf,dn,dtslf> User leap future event in GPS week number, day "
      "number, next leap second e.g. 2347,3,19\n"
      "  -t <date,time>   Scenario start time YYYY/MM/DD,hh:mm:ss\n"
      "  -T <date,time>   Overwrite TOC and TOE to scenario start time\n"
      "  -d <duration>    Duration [sec] (dynamic mode max: %.0f, static mode "
      "max: %d)\n"
...
int main(int argc, char *argv[]) {
  ...
  while ((result = getopt(argc, argv,
    ...
    case 'p':
      if (optind < argc &&
          argv[optind][0] != '-') // Check if next item is an argument
      {
    ...
    case 'A':
      if (parseAttackConfig(&attack_cfg, optarg) == FALSE) {
        fprintf(
            stderr,
            "ERROR: Invalid attack spec. Use PRN:method[,PRN:method...].\n");
        fprintf(stderr, "       Methods: "
                        "normal,jam_drop,jam_noise,spoof_delay,spoof_nav\n");
        exit(1);
      }
      attack_enabled = TRUE;
      break;
  ...
  // Open output as early as possible so stream outputs can block on the
  // reader before the "now" scenario time is latched.
  if (strcmp("-", outfile)) {
  ...
  if (attack_enabled == TRUE) {
    ...
    if (hasUnimplementedAttack(&attack_cfg) == TRUE)
      fprintf(stderr, "WARNING: Some configured methods are placeholders and "
  ...
  for (iumd = 1; !stop_requested && (stream_forever == TRUE || iumd < numd);
       iumd++) {
    int motion_index = staticLocationMode ? 0 : iumd;
    ...
    for (i = 0; i < MAX_CHAN; i++) {
      if (chan[i].prn > 0) {
        ...
        // Current pseudorange
        if (!staticLocationMode)
          computeRange(&rho, active_eph[sv], &ionoutc, grx, xyz[motion_index]);
        else
    ...
    for (isamp = 0; isamp < iq_buff_size; isamp++) {
      ...
      for (i = 0; i < MAX_CHAN; i++) {
        if (chan[i].prn > 0) {
          ...
          attack_method = ATTACK_METHOD_NONE;
          if (attack_enabled == TRUE)
    ...
    if (data_format == SC01) {
      ...
      if (writeOutput(fp, iq8_buff, 1, iq_buff_size / 4) == FALSE) {
        fprintf(stderr, "\nERROR: Failed to write output samples.\n");
        stop_requested = 1;
        break;
      }
      ...
      if (writeOutput(fp, iq8_buff, 1, 2 * iq_buff_size) == FALSE) {
        fprintf(stderr, "\nERROR: Failed to write output samples.\n");
        stop_requested = 1;
        break;
      }
    ...
    {
      if (writeOutput(fp, iq_buff, 2, 2 * iq_buff_size) == FALSE) {
        fprintf(stderr, "\nERROR: Failed to write output samples.\n");
        stop_requested = 1;
        break;
      }
